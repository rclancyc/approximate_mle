...
for model = 1:4
    ...
    for jj = 1:length(ms) 
        ...
        for ii = 1:n_sims  
            ...
            switch model
                case 1
                    W = diag([ones(1,N)*lambda, 1/sigma]);
                case 2
                    W = diag([ones(1,N)/rho, 1/sigma]);
                    neg_L =@(z) gaussian_neg_loglikelihood(y, H, z, rho, sigma);
                case 3
                    W = diag([ones(1,N)/sqrt((2*c)^2/12), 1/sigma]);
                    neg_L =@(z) round_neg_loglikelihood(y, H, z, c, sigma);
                case 4
                    W = diag([ones(1,N)/sqrt(25e4), 1/sigma]);
                    D = D_array{ii}(1:m,:);
                    neg_L =@(z) float_neg_loglikelihood(y, H, z, D, sigma);
            end            
            
            
            %W = diag(logspace(-2,4,N+1));
            %W = diag([ones(N,1); 1e32]);
            xtls = tls2(H, y, W);
            %xtls = tls3(H, y);
